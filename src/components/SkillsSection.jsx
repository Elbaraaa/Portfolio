...
function SkillCard({ cat, visible, delay, C }) {
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: "clamp(12px, 1vw, 16px)" }}>
        <div
          style={{
      ...
      {cat.items.map((skill, i) => (
        <div key={skill.name} style={{ marginBottom: i < cat.items.length - 1 ? "clamp(7px, 0.6vw, 10px)" : 0 }}>
          ...
          <div style={{ height: "clamp(4px, 0.35vw, 5px)", background: C.border, borderRadius: 2, overflow: "hidden" }}>
            <div
              style={{
                height: "100%",
  ...
  return (
    <section
      ...
      style={{
        ...
        minHeight: "100vh",
        display: "flex",
        alignItems: phase === "grid" || isClosing ? "flex-start" : "center",
        paddingTop: phase === "grid" || isClosing ? "clamp(96px, 8vw, 140px)" : undefined,
        position: "relative"
      }}
    >
      <div
      ...
      >
        ...
        >
          <div style={{ width: "clamp(28px, 2vw, 38px)", height: 1, background: `linear-gradient(90deg,${C.accent},${C.green})` }} />
          <span
            style={{
              fontFamily: "monospace",
              fontSize: "clamp(10px, 0.75vw, 12px)",
        ...
        {isMobile && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: "clamp(16px, 1.2vw, 20px)" }}>
        ...
        >
          ...
          >
            {skillCategories.map((cat, i) => (
              <SkillCard key={cat.category} cat={cat} visible={cardsVisible} delay={i * 0.1} C={C} />
            ))}
          </div>
