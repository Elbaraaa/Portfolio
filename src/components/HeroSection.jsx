...
export function HeroSection({ onMenu, onHire, C }) {
  ...
  return (
    <section id="hero" style={{position:"relative",height:"100vh",display:"flex",alignItems:"center",overflow:"hidden",background:"transparent"}}>
      ...
      >
        <div style={{fontFamily:"monospace",fontSize:10,color:C.textDim,textTransform:"uppercase",letterSpacing:"0.1em",marginBottom:12}}>
          System
        </div>

        {[["BUILD","PASSING",C.green],["DEPLOY","LIVE",C.accent],["SESSION",uptime+"s",C.orange]].map(row=>(
          <div key={row[0]} style={{display:"flex",justifyContent:"space-between",marginBottom:9,fontFamily:"monospace",fontSize:12}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:6,fontFamily:"monospace",fontSize:11,color:C.green,marginBottom:12}}>
      ...
      >
        ...
        <h1 style={{fontFamily:"system-ui,-apple-system",fontWeight:800,lineHeight:0.92,margin:"0 0 20px",letterSpacing:"-0.03em",animation:"fadeUp 0.7s 0.4s both"}}>
          <span className="grad-text" style={{display:"block",fontSize:"clamp(48px,7vw,82px)",background:`linear-gradient(135deg,${C.textPrimary},${C.accent})`}}>
            Elbaraa
          </span>
          <span className="grad-text" style={{display:"block",fontSize:"clamp(48px,7vw,82px)",background:`linear-gradient(135deg,${C.accent},${C.green})`}}>
            Abdalla.
          </span>
        </h1>

        <div style={{fontFamily:"monospace",fontSize:14,color:C.accent,marginBottom:18,minHeight:22,animation:"fadeUp 0.5s 0.9s both"}}>
          <Typewriter text={`> ${personal.subTagline}`} delay={1000}/>
        </div>

        <p style={{color:C.textSecondary,fontSize:15,lineHeight:1.75,maxWidth:500,marginBottom:32,animation:"fadeUp 0.6s 1.1s both"}}>
        ...
        <div style={{fontFamily:"monospace",fontSize:10,color:C.textDim,marginBottom:24,animation:"fadeUp 0.5s 3s both",display:"flex",alignItems:"center",gap:8}}>
          ...
          <span>
            type <span style={{color:C.accent,borderBottom:`1px dashed ${C.accent}50`}}>elbaraa</span> anywhere to open the secret terminal
          </span>
        </div>
        ...
        <div style={{display:"flex",gap:12,marginBottom:48,animation:"fadeUp 0.5s 1.2s both",flexWrap:"wrap"}}>
          <button
            onClick={onMenu}
            style={{display:"inline-flex",alignItems:"center",gap:8,padding:"12px 26px",background:`linear-gradient(135deg,${C.accent},${C.green})`,color:C.bg,fontWeight:700,fontSize:13,borderRadius:7,border:"none",cursor:"pointer"}}
          >
            Explore Work ↓
          </button>
          ...
          <button
            onClick={onHire}
            style={{display:"inline-flex",alignItems:"center",gap:8,padding:"12px 26px",border:`1px solid ${C.border}`,color:C.textSecondary,fontSize:13,borderRadius:7,background:"none",fontFamily:"monospace",cursor:"pointer"}}
          >
            ✉️ Hire Me
          </button>
          ...
          <button
            ...
            onClick={()=>alert("📄 Resume link coming soon!")}
            style={{alignItems:"center",gap:8,padding:"12px 20px",background:`linear-gradient(90deg,${C.accent}20,${C.green}15)`,border:`1px solid ${C.accent}40`,borderRadius:7,color:C.accent,fontFamily:"monospace",fontSize:12,cursor:"pointer",fontWeight:700}}
          >
            📄 Resume
          </button>
        </div>
        ...
        <div className="hero-stats-grid" style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",border:`1px solid ${C.border}`,borderRadius:10,overflow:"hidden",animation:"fadeUp 0.6s 1.5s both",maxWidth:520}}>
          {stats.map((s,i)=>(
            ...
            >
              <div className="grad-text" style={{fontFamily:"system-ui",fontSize:22,fontWeight:800,background:`linear-gradient(135deg,${C.textPrimary},${C.accent})`,marginBottom:3}}>
                {s.value}
              </div>
              <div style={{fontFamily:"monospace",fontSize:8,color:C.textDim,textTransform:"uppercase",letterSpacing:"0.08em",lineHeight:1.3}}>
                {s.label}
              </div>
            </div>
          ))}
        ...
        {isTightHero && (
          ...
          >
            <div style={{fontFamily:"monospace",fontSize:10,color:C.textDim,marginBottom:6}}>
              scroll to navigate
            </div>
