...
function Game2048({ C, onClose }) {
  // Responsive tile size based on viewport
  const tileSize = Math.min(68, Math.floor((Math.min(window.innerWidth, 480) - 80) / 4));
  const CELL = Math.max(4, Math.floor(tileSize * 0.08));
  const PAD = Math.max(6, Math.floor(tileSize * 0.1));
  const TILE = tileSize;
  ...
  const boardToTiles = b => {
    const t = [];
    b.forEach((r,i) => r.forEach((v,j) => { if (v) t.push({id:mkId(),value:v,row:i,col:j,merged:false,fresh:true}); }));
    return t;
  };
  ...
  const slide = row => {
    ...
    for (let i = 0; i < f.length - 1; i++) {
      if (f[i] === f[i+1]) { f[i] *= 2; s += f[i]; f[i+1] = 0; }
    }
    f = f.filter(v => v);
  ...
  const move = useCallback(dir => {
    ...
    setTiles(tiles => {
      ...
      if (dir === "left") {
        ...
        // Fix: compare slid-reversed row with original reversed row (not double-reversed)
        nb = rv(nb).map(r => {
          const res = slide(r);
          if (res.row.join() !== r.join()) mv = true;
          sc += res.score;
        ...
        // down: Fix same as right — compare with the reversed row, not the original
        nb = tr(nb);
        nb = rv(nb).map(r => {
          const res = slide(r);
          if (res.row.join() !== r.join()) mv = true;
      ...
      if (!hm) setOver(true);
      const newTiles = [];
      wt.forEach((r,i) => r.forEach((v,j) => {
        if (v) newTiles.push({id:mkId(),value:v,row:i,col:j,merged:nb[i][j]!==grid[i][j]&&v>2,fresh:grid[i][j]===0&&wt[i][j]!==0});
      }));
      return newTiles;
    });
  }, [over]);
  ...
  const tc = v => ({
    0:"transparent",2:"#7C6FFF",4:"#6B5FEE",8:"#4AFFC4",16:"#3EEEBB",
    32:"#F97FFF",64:"#E870EE",128:"#FFB347",256:"#FF9520",512:"#FF7B00",
    1024:"#7C6FFF",2048:"#4AFFC4"
  }[v] || "#FFD700");
  ...
  return (
    ...
    >
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
        <div style={mkPanel(C,{padding:"8px 14px"})}>
          <div style={{fontFamily:"monospace",fontSize:9,color:C.textDim}}>SCORE</div>
          <div style={{fontFamily:"monospace",fontSize:16,fontWeight:700,color:C.textPrimary}}>{score}</div>
        </div>
        <div style={{display:"flex",gap:8}}>
          ...
          <button onClick={onClose} style={{fontFamily:"monospace",fontSize:11,padding:"7px 14px",border:`1px solid ${C.border}`,borderRadius:6,background:"none",color:C.textDim,cursor:"pointer"}}>✕ Close</button>
        </div>
      </div>
      ...
      <div style={{position:"relative",width:boardSize,height:boardSize,background:C.bg,borderRadius:10,border:`1px solid ${C.border}`,padding:PAD,margin:"0 auto"}}>
        {Array(16).fill(0).map((_,i) => (
          <div key={i} style={{position:"absolute",width:TILE,height:TILE,borderRadius:6,background:"#1A2540",left:PAD+(i%4)*(TILE+CELL),top:PAD+Math.floor(i/4)*(TILE+CELL)}}/>
        ))}
        {tiles.map(t => (
      ...
      <div style={{fontFamily:"monospace",fontSize:9,color:C.textDim,marginTop:8,textAlign:"center"}}>
...
export function ArcadeSection({ C }) {
  ...
  // Lock background scroll while game overlay is open
  useEffect(() => {
  ...
  return (
    <section id="arcade" ref={ref} className="section-pad" style={{padding:"80px 60px",background:"transparent",minHeight:"100vh",display:"flex",alignItems:"center",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{maxWidth:1080,margin:"0 auto",width:"100%"}}>
        ...
        <div className="arcade-cards-grid" style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:16}}>
          {games.map((g,i) => (
            <div key={g.name} onClick={() => g.name==="2048" && setPlaying(true)}
              ...
              <div className="arcade-card-icon" style={{fontSize:48,marginBottom:16}}>{g.icon}</div>
              <h3 style={{fontFamily:"system-ui",fontWeight:800,fontSize:"clamp(16px,3vw,22px)",color:C.textPrimary,marginBottom:8}}>{g.name}</h3>
      ...
      {/* Game overlay modal — shown over the page, not embedded */}
      {playing && (
        <div
        ...
        >
          ...
          >
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
              ...
              <div style={{fontFamily:"monospace",fontSize:10,color:C.textDim}}>tap outside to close</div>
            </div>
